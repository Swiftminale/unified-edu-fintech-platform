  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-extrabold text-slate-900 tracking-tight">Billing & Invoicing</h2>
          <p className="text-slate-500 text-sm mt-1 font-medium">Configure tuition fees and generate bulk invoices for grades.</p>
        </div>
      ...
      <Card className="bg-white border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-3xl overflow-hidden">
        <CardHeader className="border-b border-slate-50 pb-4">
          ...
          <CardDescription className="text-slate-500 font-medium ml-10">List of active billing definitions.</CardDescription>
        </CardHeader>
            ...
            <Table>
              <TableHeader className="bg-slate-50">
                <TableRow className="hover:bg-slate-50 border-none">
                  <TableHead className="text-slate-500 font-bold uppercase tracking-wider text-xs pl-8">Name</TableHead>
                  <TableHead className="text-slate-500 font-bold uppercase tracking-wider text-xs">Amount</TableHead>
                  <TableHead className="text-slate-500 font-bold uppercase tracking-wider text-xs">Cycle</TableHead>
                  <TableHead className="text-slate-500 font-bold uppercase tracking-wider text-xs text-right pr-8">Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {feeStructures.map(fee => (
                  <TableRow key={fee.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    <TableCell className="font-bold text-slate-800 pl-8">{fee.name}</TableCell>
                    <TableCell className="text-slate-900 font-bold">ETB {fee.amount.toFixed(2)}</TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right pr-8">
                      {generateResult?.id === fee.id ? (
                        <div className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-full text-sm font-bold border border-emerald-100">
                        ...
                        <Button
                          ...
                          className="bg-purple-600 hover:bg-purple-500 text-white rounded-full shadow-md font-semibold transition-all active:scale-95 px-5"
                        >
