  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-extrabold text-slate-900 dark:text-zinc-100 tracking-tight">Grades & Classes</h2>
          <p className="text-slate-500 dark:text-zinc-400 text-sm mt-1 font-medium">Manage academic levels and physical class groupings.</p>
        </div>
        
        <div className="flex items-center gap-3">
          <Dialog open={isBulkGradeModalOpen} onOpenChange={setIsBulkGradeModalOpen}>
            <DialogTrigger asChild>
              <Button className="bg-purple-600 dark:bg-purple-700 hover:bg-purple-500 dark:hover:bg-purple-600 text-white rounded-xl shadow-sm px-6 font-semibold h-11">
                <Layers className="h-4 w-4 mr-2" /> Bulk Add Grades
              </Button>
            </DialogTrigger>
            <DialogContent className="bg-white dark:bg-zinc-950 border-none shadow-2xl text-slate-900 dark:text-zinc-100 rounded-3xl p-8 max-w-4xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
              ...
              <form onSubmit={handleBulkCreateGrades} className="space-y-6 mt-4">
                <div className="w-64">
                  <select
                    ...
                    className="w-full bg-slate-50 dark:bg-zinc-900 border-slate-200 dark:border-zinc-800 border rounded-2xl px-4 py-3 text-sm text-slate-900 dark:text-zinc-100 focus-visible:ring-2 focus-visible:ring-purple-200 dark:focus-visible:ring-purple-900 outline-none appearance-none"
                ...
                <div className="space-y-4">
                  {bulkGrades.map((row, idx) => (
                    <div key={idx} className="flex flex-col sm:flex-row gap-3 items-start sm:items-center bg-slate-50 dark:bg-zinc-900/50 p-4 rounded-2xl border border-slate-100 dark:border-zinc-800">
                      <Input 
                        placeholder="Grade Name (e.g. Grade 1)" 
                        required 
                        value={row.name} 
                        onChange={e => updateBulkRow(idx, 'name', e.target.value)} 
                        className="bg-white dark:bg-zinc-900 border-slate-200 dark:border-zinc-800 text-slate-900 dark:text-zinc-100" 
                      />
                      <Input 
                      ...
                      <Input 
                        ...
                        className="bg-white dark:bg-zinc-900 border-slate-200 dark:border-zinc-800 text-slate-900 dark:text-zinc-100 w-full sm:w-32" 
                      />
                      <select
                        required
                        value={row.billingCycle}
                        onChange={e => updateBulkRow(idx, 'billingCycle', e.target.value)}
                        className="w-full sm:w-40 bg-white dark:bg-zinc-900 border-slate-200 dark:border-zinc-800 border rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-zinc-100 outline-none appearance-none h-10"
                      ...
                      <Button 
                        ...
                        className="text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/40 px-3"
                      >
                        <Trash2 className="h-4 w-4" />
                      </Button>
                    </div>
                  ))}
                  <Button type="button" variant="outline" onClick={addBulkRow} className="w-full border-dashed border-2 text-slate-500 dark:text-zinc-400 font-bold rounded-2xl h-12">
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-slate-500 dark:text-zinc-500 font-medium flex flex-col items-center bg-white dark:bg-zinc-950 rounded-3xl shadow-sm border border-slate-100/50 dark:border-zinc-900">
          <div className="w-16 h-16 bg-slate-50 dark:bg-zinc-900 rounded-2xl flex items-center justify-center mb-4">
            <BookOpen className="h-8 w-8 text-slate-300 dark:text-zinc-600" />
          </div>
        ...
        <div className="space-y-6">
          {grades.map(grade => (
            <Card key={grade.id} className="bg-white dark:bg-zinc-950 border-none shadow-sm rounded-3xl overflow-hidden p-2">
              <CardHeader className="flex flex-row items-center justify-between pb-4 border-b border-slate-100 dark:border-zinc-900">
                <CardTitle className="text-xl font-bold text-slate-800 dark:text-zinc-100">{grade.name}</CardTitle>
                <Button
                  variant="outline"
                  size="sm"
                  className="border-slate-200 dark:border-zinc-800 text-slate-600 dark:text-zinc-300 hover:text-slate-900 dark:hover:text-zinc-100 hover:bg-slate-50 dark:hover:bg-zinc-900 rounded-xl font-semibold h-10"
                  onClick={() => {
                    setSelectedGradeId(grade.id);
                    setIsClassModalOpen(true);
                  }}
                >
                  <PlusCircle className="h-4 w-4 mr-2 text-purple-500 dark:text-purple-400" /> Add Class to {grade.name}
                </Button>
              </CardHeader>
                ...
                {grade.classes && grade.classes.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {grade.classes.map((cls: any) => (
                      <div key={cls.id} className="p-4 rounded-2xl bg-slate-50 dark:bg-zinc-900/50 border border-slate-100 dark:border-zinc-800 flex flex-col gap-1 group hover:shadow-md dark:hover:shadow-[0_0_15px_rgba(168,85,247,0.1)] transition-all duration-300">
                        <span className="font-bold text-slate-800 dark:text-zinc-200 text-lg">{cls.name}</span>
                        <span className="text-xs text-slate-400 dark:text-zinc-500 font-semibold tracking-wider uppercase">ID: {cls.id.slice(-6)}</span>
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-slate-400 dark:text-zinc-600 text-sm font-medium italic text-center py-4 bg-slate-50 dark:bg-zinc-900/30 rounded-2xl">No classes added to this grade yet.</p>
                )}
              </CardContent>
            </Card>
          ))}
